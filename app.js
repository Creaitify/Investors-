...
function bindInteractions() {
  ...
  const fieldRules = {
    name: {
      test: value => /^[A-Za-z][A-Za-z.'-]*(?:\s+[A-Za-z][A-Za-z.'-]*)+$/.test(value),
    ...
    email: {
      test: value => /^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}$/.test(value),
      message: 'Please enter a valid email format (example: name@example.com).',
    },
    city: {
      test: value => /^[A-Za-z][A-Za-z\s.'-]{1,49}$/.test(value),
      message: 'Please enter your city.',
    },
  };
  const showFieldError = (input,message) => {
    const slot = $('.field-error',input.closest('label,.modal-field'));
  ...
  const validateField = input => {
    const rule = fieldRules[input.dataset.validate];
    if (!rule) return true;
    const value = input.value.trim();
    const valid = value ? rule.test(value) : false;
    showFieldError(input,valid ? '' : rule.message);
    return valid;
  };
  $$('form').forEach(form => {
    ...
    form.addEventListener('submit', e => {
      ...
      const enquiry = $('.custom-select-value',form), enquiryTrigger = $('.custom-select-trigger',form);
      if (enquiry && !enquiry.value) { enquiryTrigger.setAttribute('aria-invalid','true'); enquiryTrigger.focus(); return; }
      form.innerHTML = '<div class="submitted">Thank you. In the full experience, a Finnovate specialist would be in touch shortly.</div>';
    });
  });
