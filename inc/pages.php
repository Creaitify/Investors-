<?php
/**
 * Page composition — ported 1:1 from investorPage() in the former app.js.
 *
...
function reviewCards()
{
    ...
    foreach (REVIEWS as $i => $x) {
        $out .= '<article class="orbit-review" data-review-index="' . $i . '"><span class="review-number">0' . ($i + 1) . '</span><blockquote>' . $x[0] . '</blockquote><figcaption><span class="review-avatar">' . $x[3] . '</span><span><b>' . $x[1] . '</b><small>' . $x[2] . '</small></span></figcaption></article>';
    }
    return $out;
}

/** Decorative curve inside the review orbit. */
